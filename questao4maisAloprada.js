...
const realizaTransfusao = (pacientes) => {
  ...
  pacientesInternados.forEach((paciente) => {
    switch (paciente.tipoSanguineo) {
      case "AB":
        if (verificaEstoque("AB")) {
          break;
        }
      case "A":
        if (verificaEstoque("A")) {
          break;
        }
      case "B":
        if (paciente.tipoSanguineo != "A" && verificaEstoque("B")) {
          break;
        }
      case "O":
        if (verificaEstoque("O")) {
          break;
        }
      default:
        console.log(
          `O paciente ${paciente.nome} com tipo sanguineo ${paciente.tipoSanguineo} nao conseguiu transfusao`
        );
        break;
    }
  });
...
const verificaEstoque = (tipoSanguineo) => {
  if (estoque[tipoSanguineo] > 0) {
    estoque[tipoSanguineo]--;
    console.log(
      `O paciente ${paciente.nome} com tipo sanguineo ${paciente.tipoSanguineo} conseguiu tipo ${tipoSanguineo}`
    );
    return true;
  }
  return false;
